    ...
    if st.button("Run pipeline"):
        with st.spinner("Running the pipeline... Please wait."):
            ...
            if xml_path:
                ...
                # Choose the correct model option based on selection
                if processing_model == "OpenAI GPT-4o (faster, more accurate)":
                    # Use the exact string "4o" which is recognized in load_modules.py
                    ff_model = "4o"  # This is the model identifier for GPT-4o in load_modules.py
                    st.info("Using OpenAI GPT-4o for paper processing")
                else:
                    ff_model = "llama3.1I-8b-q4"  # Use local model
                    st.info("Using local Llama model for paper processing")
                
                # Pass the model identifier directly to run_pipeline
                output_json = run_pipeline(xml_path, schema_path, ff_model=ff_model)
